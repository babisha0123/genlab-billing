  ...
  invoicePreview.innerHTML = `
    ...
    <ul>
      ${invoice.items
        .map(
          (item) =>
            `<li>${item.name}: ${item.quantity} × ${item.unitPrice.toFixed(
              2
  ...
  return `
    <article class="invoice-card">
      ...
      <div class="card-actions">
        <button onclick="toggleStatus('${invoice._id}', '${invoice.paymentStatus}')">
          Toggle Status
        </button>
        ...
        <button onclick="sendEmail('${invoice._id}')">
          Send Email
        </button>
      </div>
    </article>
...
async function fetchInvoices() {
  try {
    ...
    console.error(error);
    invoiceList.innerHTML = "<p>Unable to load invoices.</p>";
  }
}
...
async function toggleStatus(id, currentStatus) {
  try {
    ...
    const response = await fetch(`${API_BASE}/invoices/${id}/payment-status`, {
      method: "PATCH",
...
invoiceForm.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    console.error(error);
    formMessage.textContent = "Invoice creation failed.";
  }
});
...
document
  .getElementById("refresh-invoices")
